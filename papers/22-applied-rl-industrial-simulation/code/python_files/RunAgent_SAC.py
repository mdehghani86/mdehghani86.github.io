# %%
from RL_Utils.Agent_SoftActorCritic import Agent
...
try:
    ...
    scores_array = pickle.load(open('./results/SAC/scores_array.pickle', 'rb'))
    loss_array = pickle.load(open('./results/SAC/loss_array.pickle', 'rb'))
    cycletime_array = pickle.load(
        open('./results/SAC/cycletime_array.pickle', 'rb'))
    eps_array = pickle.load(open('./results/SAC/eps_array.pickle', 'rb'))

except:
    run_num = 0
    env = simio_env_continuous()
    agent = Agent(num_actions=1, input_dims=env.observation_space.shape,
                  alr=alr, blr=blr, gamma=gamma, mem_size=mem_size, batch_size=batch_size,
    ...
    pickle.dump(scores_array, open('./results/SAC/scores_array.pickle', 'wb'))
    pickle.dump(loss_array, open('./results/SAC/loss_array.pickle', 'wb'))
    pickle.dump(cycletime_array, open(
        './results/SAC/cycletime_array.pickle', 'wb'))
    pickle.dump(eps_array, open('./results/SAC/eps_array.pickle', 'wb'))
...
if(run_num > 0):
    ...
    actor_loss, critic_loss, value_loss = agent.train()
    loss = [actor_loss, critic_loss, value_loss]
